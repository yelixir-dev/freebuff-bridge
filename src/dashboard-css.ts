...
.wrap{width:min(100% - 20px,1120px);margin:0 auto;padding:10px 0 24px}
.top{position:relative;display:flex;gap:24px;align-items:flex-start;justify-content:space-between;margin-bottom:18px;background:var(--ink);box-shadow:var(--shadow);color:var(--paper);padding:28px}
.top::after{content:"";position:absolute;inset:9px;border:1px solid rgba(255,253,248,.28);pointer-events:none}
...
.lang-btn{min-height:32px;min-width:36px;padding:4px 6px;background:transparent;color:var(--paper);border:1px solid rgba(255,253,248,.35);filter:grayscale(1);opacity:.65}
...
.pill{display:inline-flex;align-items:center;gap:7px;border-left:2px solid var(--rust);padding:4px 0 4px 9px}
...
.card{background:var(--panel);border-top:3px solid var(--ink);box-shadow:var(--shadow);padding:clamp(16px,3vw,28px)}
.card h2{display:flex;align-items:baseline;justify-content:space-between;gap:16px;flex-wrap:wrap;font-size:clamp(20px,2.4vw,28px);letter-spacing:-.025em;margin:0 0 16px;border-bottom:1px solid var(--line);padding-bottom:9px}
.sub{color:var(--muted);font:600 11px/1.3 var(--sans);letter-spacing:.04em;text-align:right}
.row{display:flex;flex-wrap:wrap;gap:8px;align-items:center}
.field{display:grid;gap:5px;text-align:left}
.field label{font:800 10px/1 var(--sans);letter-spacing:.09em;color:var(--muted);text-transform:uppercase}
.bind-grid{display:grid;grid-template-columns:minmax(0,2fr) minmax(92px,1fr);gap:10px;align-items:end}
.bridge-key{margin:12px 0 0}
.bridge-key-row{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:center}
.bridge-key-prefix{min-height:42px;display:inline-flex;align-items:center;border:1px solid var(--line);border-right:0;background:var(--paper-2);padding:0 10px;font-family:var(--mono);font-weight:700}
.bridge-key-row input{min-width:0;border-left:0}
.bridge-key-help-row{display:grid;grid-template-columns:minmax(0,1fr) auto auto auto;gap:7px;align-items:center;margin-top:7px}
.bridge-key-help{font:700 11px/1.2 var(--sans);color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.bridge-key-help-row button{min-height:36px;min-width:38px;padding:5px 8px}
...
input,select{width:100%;min-height:42px;border:1px solid var(--line);border-radius:0;background:var(--paper);color:var(--ink);padding:9px 10px}
...
.policy{position:relative;border:1px solid var(--line);border-left:3px solid transparent;padding:11px 12px;background:var(--paper);display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;text-align:left}
...
.model{display:flex;align-items:center;justify-content:space-between;gap:12px;border:1px solid var(--line);background:var(--paper);padding:11px}
...
.slider:before{content:"";position:absolute;width:18px;height:18px;left:3px;top:3px;background:var(--muted)}
...
.credential-fold summary,.provider-fold summary{display:flex;align-items:center;gap:9px;padding:11px;cursor:pointer;list-style:none}
...
.credential-body,.provider-models{display:grid;gap:8px;padding:11px}
.small{font:12px/1.45 var(--sans);color:var(--muted);overflow-wrap:anywhere}
.footerbar{margin:0 0 16px;background:var(--ink);padding:10px;display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center}
...
.footerbar #restart.active,.footerbar #restart:hover{background:var(--gold);color:var(--ink);border-color:var(--gold)}
.token{font-family:var(--mono);font-size:11px;color:var(--paper)}
.toast{position:fixed;left:10px;right:10px;top:10px;background:var(--ink);color:var(--paper);border-left:4px solid var(--gold);padding:12px;transform:translateY(-140%);transition:transform .2s;z-index:20}
.toast.show{transform:translateY(0)}
.add-row{display:grid;grid-template-columns:1fr auto;gap:8px}
@media(max-width:520px){.footerbar{grid-template-columns:1fr 1fr}.footerbar .token{grid-column:1/-1}.add-row{grid-template-columns:1fr}.bridge-key-help{display:none}.bridge-key-help-row{grid-template-columns:repeat(3,1fr)}}
@media(max-width:560px){.brand h1{white-space:normal}.bind-grid{grid-template-columns:1fr}.bridge-key-help-row{grid-template-columns:1fr repeat(3,auto)}}
@media(min-width:760px){.wrap{padding:22px 0 36px}.grid{grid-template-columns:1fr 1fr}.wide{grid-column:1/-1}.brand h1{font-size:clamp(30px,4vw,48px)}.top{align-items:center;padding:clamp(32px,5vw,54px)}.provider-models{grid-template-columns:repeat(2,minmax(0,1fr))}.creds{grid-template-columns:repeat(2,minmax(0,1fr))}}
